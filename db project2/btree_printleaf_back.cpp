...
public:
	...
	int insert(int key, int val) {
		...
		//make first nonleaf and leaf node
			//and set nonleaf as root
		if (depth == 0) {
		...
		if (lf->entry[lf->sz - 1].__key == 0) {
			lf->writeNode(fs, headerSize, blockSize);
		}
		// leafNode is full -> split node
		else {
			//split leaf node
			int split_idx = lf->sz / 2;
			newleaf = new leafNode(entrySize, availBid++);
			for (int i = split_idx, newidx = 0; i < lf->sz; i++, newidx++) {
				newleaf->entry[newidx] = lf->entry[i];
				lf->entry[i] = emptyElem;
			}
			Entry newEntry = newleaf->entry[0];
			...
			for (auto iter = path.rbegin(); iter != path.rend(); iter++) {
				...
				if (cur->entry[cur->sz - 1].__key != NULL) {
					int split_idx = cur->sz / 2;
					...
					for (int i = split_idx + 1, newidx = 0; i < cur->sz; i++, newidx++) {
						if (i != split_idx)
